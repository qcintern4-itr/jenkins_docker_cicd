...
try:
    ...
    from project_bioflux_ai_report_portal.config.web_locators import study_management as locators
    from selenium.webdriver import Keys
...
class StudyManagement(WebPage):
    ...
    def get_study_details_callcenter(self):
        ...
        if self.browser.wait_visibility_of_element_located(self.LCT.STDET_CONTENT_ARTIFACTTEXT):
            ...
            output['STUDY INFORMATION'] = {study_info[i].replace(':', ''): study_info[i + 1] for i in range(0, len(study_info), 2)}
            output['STUDY INFORMATION'].update({'Artifact Tag': artifact_tag})  # Add tag into list
        else:
            output['STUDY INFORMATION'] = {study_info[i].replace(':', ''): study_info[i + 1] for i in range(0, len(study_info), 2)}
        study_setting = root_output['STUDY SETTINGS']
        output['STUDY SETTINGS'] = {study_setting[i].replace(':', ''): study_setting[i + 1] for i in range(0, len(study_setting), 2)}
        ...
        output['INTERPRETING PHYSICIAN'] = {inter_phy[i].replace(':', ''): inter_phy[i + 1] for i in range(0, len(inter_phy), 2)}
    ...
    def get_all_study_events(self):
        # self.wait_until_study_events_available()
        self.browser.wait_visibility_of_element_located(self.LCT.EVENT_CONTENT_ROOT, inverse=True)
        output = list()
        start_time = time.time()
        elements = len(self.browser.get_elements(self.LCT.EVENT_CONTENT_ROOT))
        if self.browser.wait_visibility_of_element_located(self.LCT.EVENT_LOADING_MORE) and self.browser.get_element(self.LCT.EVENT_LOADING_MORE).is_displayed():
            self.browser.scroll_into_view(self.LCT.EVENT_LOADING_MORE)
            self.browser.wait_visibility_of_element_located(self.LCT.EVENT_LOADING_MORE, inverse=True)
        root_elements = self.browser.get_elements(self.LCT.EVENT_CONTENT_ROOT)
        while True:
            if len(root_elements) > elements:
                self.browser.scroll_into_view(self.LCT.EVENT_LOADING_MORE)
                self.browser.wait_visibility_of_element_located(self.LCT.EVENT_LOADING_MORE, inverse=True)
                root_elements = self.browser.get_elements(self.LCT.EVENT_CONTENT_ROOT)
                elements += 10
            elif len(root_elements) <= elements:
                total_elements = self.browser.get_elements(self.LCT.EVENT_CONTENT_ROOT)
                for root_elem in total_elements:
    ...
    def edit_study_event_on(self, **on):
        ...
        self.browser.js_clicks(self.LCT.EVENT_EDIT, on_elements=lambda i, _: i == _index, stop_on_first=True)
    ...
    def get_number_events_by_event_type(self, event_type=None):
        self.filter_study_events(by_type=event_type)
        numbers_event = self.get_all_study_events()
        return len(numbers_event)
