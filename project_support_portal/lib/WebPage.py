...
try:
    ...
    from project_support_portal.lib import ConfigHandler
...
class WebPage(WebPageModel):
    ...
    def __init__(self, browser, env, locators):
        super().__init__(project_name='support_portal', browser=browser, env=env, locators=locators)
        self.cfg = ConfigHandler.ConfigHandler(self.env)
        self.URL = self.cfg.params.get('WEB_BASE_URL')
        self.URL_DGT = self.cfg.params.get('WEB_DIGITALIZATION_URL')
