...
try:
    ...
    from project_cardiac_portal.config.web_locators import devices_notifications as locators
...
class DevicesNotifications(WebPage):
    """
    Handle Heart monitor > Notification
    """
    ...
    def navigate_to_devices_notifications(self):
        self.browser.open_page(self.URL + '/heartmonitor/hm-notification')

    def filter_devices_notifications(self, by_priority=None, by_patient=None, by_status=None, search_by=None,
                                     text_search=None):
        if by_priority:
            self.browser.click(self.LCT.PRIORITY)
            ...
            self.browser.clicks(self.LCT.PRIORITY_OPTIONS,
                                on_elements=lambda _, e: self.browser.get_text(e).lower() == by_priority.lower(),
                                stop_on_first=True)
        if by_patient:  # available for physician only
        ...
        if search_by:
            ...
            self.browser.clicks(self.LCT.SEARCHBY_OPTIONS,
                                ...
                                stop_on_first=True)
            Utils.wait_until(func=self.browser.get_attribute_value, args=(self.LCT.SEARCH_TEXTBOX, 'placeholder'),
        ...
        self.browser.wait_invisibility_of_element_located('selector:div.ant-spin-nested-loading', timeout=3)
        self.browser.wait_visibility_of_all_elements_located(self.LCT.CONTENT)

    def get_devices_notifications(self, row=None):
        self.browser.wait_visibility_of_all_elements_located(self.LCT.HEADER)
        self.browser.wait_visibility_of_all_elements_located(self.LCT.CONTENT, timeout=5)
    ...
    def _on_item(self, **on):
        if 'row' in on:
            return int(on['row']) - 1
        data = self.get_devices_notifications()
    ...
    def view_devices_notifications_appointment_summary_on(self, **on):
        self.browser.wait_visibility_of_all_elements_located(self.LCT.CONTENT)
        index_to_click = self._on_item(**on)
        self.browser.clicks(self.LCT.CONTENT_PATIENTNAME, on_elements=lambda i, _: i == index_to_click,
                            stop_on_first=True)

    def view_devices_notifications_report_details_on(self, **on):
        self.browser.wait_visibility_of_all_elements_located(self.LCT.CONTENT)
        index_to_click = self._on_item(**on)
        self.browser.clicks(self.LCT.CONTENT_ROW, on_elements=lambda i, _: i == index_to_click, stop_on_first=True)
