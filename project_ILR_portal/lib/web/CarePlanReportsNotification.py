...
try:
    ...
    from project_cardiac_portal.config.web_locators import care_plan_reports_notification as locators
...
class CarePlanReportsNotification(WebPage):

    """
    Handle Care plan reports > Notification
    """
    ...
    def navigate_to_care_plan_notification_reports(self):
        self.browser.open_page(self.URL + '/reports/notification')

    def filter_care_plan_notification_reports(self, status=None, by_patient=None, search_by=None, text_search=None):
        if status:
    ...
    def get_care_plan_notification_reports(self, row=None):
    ...
    def _on_item(self, **on):
        if 'row' in on:
            return int(on['row']) - 1
        data = self.get_care_plan_notification_reports()
    ...
    def view_care_plan_report_notification_appointment_summary_on(self, **on):
        self.browser.wait_visibility_of_all_elements_located(self.LCT.CONTENT)
        index_to_click = self._on_item(**on)
        self.browser.clicks(self.LCT.CONTENT_PATIENTNAME, on_elements=lambda i, _: i == index_to_click,
                            stop_on_first=True)

    def view_care_plan_report_notification_details_on(self, **on):
        self.browser.wait_visibility_of_all_elements_located(self.LCT.CONTENT)
        index_to_click = self._on_item(**on)
