*** Settings ***
Documentation   Biotres device patient commanded events verification test suite
Metadata        sid     BTDV-PATIENTEVENT
Metadata        owner   Nang Phung
Library         corelib/AssertionExtension.py
Library         corelib/WebHandler.py   ${BROWSER}  ${HEADLESS}

Force Tags      BTDV-PATIENTEVENT       device
Test Timeout    1h

*** Test Cases ***
BTDV-PATIENTEVENT-01: Detect user commanded (manual) cardiac event
    ...
    ...     - 6. Select any symptom then press “Send”.
    ...     Test Output:
    ...
    ...     - 3. In WinSCP, refresh the view. Within the device folder, locate the device acknowledgement of the event button pressed action. Verify there are files with the format below:
    ...
    ...     - 7. In WinSCP, refresh the view. Within the device folder, locate the device acknowledgement of the manual detected event. Verify there are files with the format below:
    ...          event-manual-mm-dd-yy-hh-mm-ss+timezone.hea
    ...          event-manual-mm-dd-yy-hh-mm-ss+timezone.dat
    ...          event-manual-mm-dd-yy-hh-mm-ss+timezone.ann
    ...     - 8. Open the event-manual-mm-dd-yy-hh-mm-ss+timezone.hea header file using text editor. Verify the “Manual event” entry and “Other” entry under the title “Reason”.
    ...     - 9. Wait for study complete.
    ...
    [Tags]  BTDV-PATIENTEVENT-01    manual
    Manual Should Be Passed     ${TEST_NAME}    ${TEST_DOCUMENTATION}

BTDV-PATIENTEVENT-02: Ignore new request to store the user commanded cardiac event
    ...
    ...     Test Output:
    ...     - 1. Verify that the Biotres gateway does not display symptoms screen.
    ...
    ...     - 4. In WinSCP, refresh the view. Within the device folder, locate the device acknowledgement of the 1st event button pressed action. Verify there are files with the format below:
    ...          event-manual-mm-dd-yy-hh-mm-ss+timezone.hea
    ...          event-manual-mm-dd-yy-hh-mm-ss+timezone.dat
    ...          event-manual-mm-dd-yy-hh-mm-ss+timezone.ann
    ...
    ...     - 7. Verify that the following error entry was created in the log file showing the rejection of storing the 2nd event press: Reject EV: MAN
    ...     - 8. Upload study data using the uploadstudy command.
    ...     - 9. Erase study by sending the erasestudy command.
    [Tags]  BTDV-PATIENTEVENT-02    manual
    Manual Should Be Passed     ${TEST_NAME}    ${TEST_DOCUMENTATION}

BTDV-PATIENTEVENT-03: Transmit cardiac event data to the server upon user commanded cardiac event record request
    [Documentation]     Author: Nang Phung
    ...
    ...     Description:
    ...     - 1. Device software shall be able to transmit cardiac event data to the server upon user commanded cardiac event record request with pre-trigger duration
    ...         equal to the programmed value of Arrhythmia Event Pre-Trigger Duration and post trigger duration equal to programmed value of Arrhythmia Event Post-Trigger Duration.
    ...     Test Setup and Input:
    ...     - 1. Complete all the steps from Section 12 "Verification Sequence & Expected Results".
    ...     - 2. Initiate the Start Study command with the parameters below:
    ...          PatientId=123456
    ...
    ...     - 7. On the symptoms screen of the Biotres gatewayBiotres gateway, select “Other” and press “Send”.
    ...     Test Output:
    ...     - 1. Wait for 2 minutes to complete the pre, post trigger recording.
    ...     - 2. Wait for additional 2 minutes for device to post the manual event data.
    ...     - 3. In WinSCP, refresh the view. Within the device folder, locate the device acknowledgement of the event button pressed action. Verify there are files with the format below:
    ...
    ...          MATLAB will plot the MIT16 file data in number of samples. The sample rate for recording is fixed at 250 samples per second. Verify the total number samples displayed by MATLAB plot is 250(sps)*(30s+30s)= 15000 samples.
    ...     - 6. Save and copy MATLAB plot to the output folder.
    ...
    ...     - 17. Erase study by sending the erasestudy command.
    [Tags]  BTDV-PATIENTEVENT-03    manual
    Manual Should Be Passed     ${TEST_NAME}    ${TEST_DOCUMENTATION}

BTDV-PATIENTEVENT-04: The device software shall be able to add symptom information
    [Documentation]     Author: Nang Phung
    ...
    ...     Description: The device software shall be able to add symptom information, which is queried via the Biotres gatewayBiotres gateway GUI, to the User commanded cardiac event.
    ...     Test Setup and Input:
    ...     - 1. Complete all the steps from Section 12 "Verification Sequence & Expected Results".
    ...     - 2. Initiate the Start Study command with the parameters below:
    ...          PatientId=123456
    ...
    ...     - 5. Once the study has started, locate and press the event button once on the device.
    ...     Test Output:
    ...     - 1. Verify pressing the “Event” button on the device, symptoms screen is displayed on the Biotres gatewayBiotres gateway.
    ...
    ...     - 6. In WinSCP, refresh the view. Within the device folder, locate the device acknowledgement of the event button pressed action. Verify there are files with the format below:
    ...          event-manual-mm-dd-yy-hh-mm-ss+timezone.hea
    ...          event-manual-mm-dd-yy-hh-mm-ss+timezone.dat
    ...          event-manual-mm-dd-yy-hh-mm-ss+timezone.ann
    ...
    [Tags]  BTDV-PATIENTEVENT-04    manual
    Manual Should Be Passed     ${TEST_NAME}    ${TEST_DOCUMENTATION}

BTDV-PATIENTEVENT-05: Detect Bradycardia
    [Documentation]     Author: Han Nguyen
    ...
    ...     Description:
    ...
    ...     - 4. In WinSCP, refresh the view. Within the device folder, locate the device acknowledgement of the auto detected event. Verify there are files with the format below:
    ...
    ...          MATLAB will plot the MIT16 file data in number of samples. The sample rate for recording is fixed at 250 samples per second. Verify the total number samples displayed by MATLAB plot is 250(sps)*(30s+30s) = 15000 samples.
    [Tags]  BTDV-PATIENTEVENT-05    manual
    Manual Should Be Passed     ${TEST_NAME}    ${TEST_DOCUMENTATION}

BTDV-PATIENTEVENT-06: Detect Tachycardia
    [Documentation]     Author: Han Nguyen
    ...
    ...     Description:
    ...     - 1. Device software shall integrate scientific algorithms (OTS) to detect non-life threating arrhythmia.
    ...
    ...     - 3. In WinSCP, refresh the view. Within the device folder, locate the device acknowledgement of the auto detected event. Verify there are files with the format below:
    ...          event-auto-mm-dd-yy-hh-mm-ss+timezone.hea
    ...          event-auto-mm-dd-yy-hh-mm-ss+timezone.dat
    ...          event-auto-mm-dd-yy-hh-mm-ss+timezone.ann
    ...
    ...     - 5. Open the event-auto-mm-dd-yy-hh-mm-ss+timezone.hea header file using MTLAB Waveform Database (WFDB) software package. Verify that the pre-trigger and post-trigger storage durations are 30 seconds each as programmed.
    ...          MATLAB will plot the MIT16 file data in number of samples. The sample rate for recording is fixed at 250 samples per second. Verify the total number samples displayed by MATLAB plot is 250(sps)*(30s+30s) = 15000 samples.
    [Tags]  BTDV-PATIENTEVENT-06    manual
    Manual Should Be Passed     ${TEST_NAME}    ${TEST_DOCUMENTATION}

BTDV-PATIENTEVENT-07: Detect Atrial Fibrillation
    [Documentation]     Author: Han Nguyen
    ...
    ...     Description:
    ...     - 1. Device software shall integrate scientific algorithms (OTS) to detect non-life threating arrhythmia.
    ...
    ...     - 5. Open the event-auto-mm-dd-yy-hh-mm-ss+timezone.hea header file using MTLAB Waveform Database (WFDB) software package. Verify that the pre-trigger and post-trigger storage durations are 30 seconds each as programmed.
    ...          MATLAB will plot the MIT16 file data in number of samples. The sample rate for recording is fixed at 250 samples per second. Verify the total number samples displayed by MATLAB plot is 250(sps)*(30s+30s) = 15000 samples.
    [Tags]  BTDV-PATIENTEVENT-07    manual
    Manual Should Be Passed     ${TEST_NAME}    ${TEST_DOCUMENTATION}

BTDV-PATIENTEVENT-08: Detect Pause
    [Documentation]     Author: Han Nguyen
    ...
    ...     Description:
    ...     - 1. Device software shall integrate scientific algorithms (OTS) to detect non-life threating arrhythmia.
    ...
    ...     - 7. Create a local "SD card" folder on the PC.
    ...
    ...     - 11. Create a local "Device" folder on the PC.
    ...
    [Tags]  BTDV-PATIENTEVENT-08    manual
    Manual Should Be Passed     ${TEST_NAME}    ${TEST_DOCUMENTATION}

BTDV-PATIENTEVENT-09: Detect Ves-Tachycardia
    [Documentation]     Author: Han Nguyen
    ...
    ...     Description: Device software shall be able to auto detect arrhythmia event using the detection algorithms.
    ...     Test Setup and Input:
    ...     - 1. Complete all the steps from Section 12 "Verification Sequence & Expected Results".
    ...     - 2. Initiate the Start Study command with the parameters below:
    ...          PatientId=123456
    ...
    ...     - 5. Once the study has started, wait for 1 minute and then using an ECG simulation tool, generate an Ves-Tachycardia event.
    ...     Test Output:
    ...
    [Tags]  BTDV-PATIENTEVENT-09    manual
    Manual Should Be Passed     ${TEST_NAME}    ${TEST_DOCUMENTATION}

BTDV-PATIENTEVENT-10: Detect Ves-Bigeminy
    [Documentation]     Author: Han Nguyen
    ...
    ...     Description: Device software shall be able to auto detect arrhythmia event using the detection algorithms.
    ...     Test Setup and Input:
    ...
    ...     - 3. Wait for 1 minute for study to get initiated on the device.
    ...     - 4. Follow all the steps on the device screen and then start the study.
    ...     - 5. Once the study has started, wait for 1 minute and then using an ECG simulation tool, generate an Ves-Bigeminy event.
    ...     Test Output:
    ...     - 1. Verify the remaining study duration on the device screen.
    ...     - 2. Wait for 5 minutes for study to get completed.
    ...     - 3. In WinSCP, refresh the view. Verify there are files with the format below:
    ...
    ...     - 8. Erase study by sending erasestudy command.
    ...     - 9. Verify device silently reboots after erasing study data.
    [Tags]  BTDV-PATIENTEVENT-10    manual
    Manual Should Be Passed     ${TEST_NAME}    ${TEST_DOCUMENTATION}

BTDV-PATIENTEVENT-11: Detect Ves-Trigeminy
    [Documentation]     Author: Han Nguyen
    ...
    ...     Description: Device software shall be able to auto detect arrhythmia event using the detection algorithms.
    ...     Test Setup and Input:
    ...
    ...     - 3. Wait for 1 minute for study to get initiated on the device.
    ...     - 4. Follow all the steps on the device screen and then start the study.
    ...     - 5. Once the study has started, wait for 1 minute and then using an ECG simulation tool, generate an Ves-Trigeminy event.
    ...     Test Output:
    ...     - 1. Verify the remaining study duration on the device screen.
    ...     - 2. Wait for 5 minutes for study to get completed.
    ...     - 3. In WinSCP, refresh the view. Verify there are files with the format below:
    ...
    ...     - 8. Erase study by sending erasestudy command.
    ...     - 9. Verify device silently reboots after erasing study data.
    [Tags]  BTDV-PATIENTEVENT-11    manual
    Manual Should Be Passed     ${TEST_NAME}    ${TEST_DOCUMENTATION}

BTDV-PATIENTEVENT-12: Detect SVES
    [Documentation]     Author: Han Nguyen
    ...
    ...     Description: Device software shall be able to auto detect arrhythmia event using the detection algorithms.
    ...     Test Setup and Input:
    ...
    ...     - 3. Wait for 1 minute for study to get initiated on the device.
    ...     - 4. Follow all the steps on the device screen and then start the study.
    ...     - 5. Once the study has started, wait for 1 minute and then using an ECG simulation tool, generate a SVES event.
    ...     Test Output:
    ...     - 1. Verify the remaining study duration on the device screen.
    ...     - 2. Wait for 5 minutes for study to get completed.
    ...     - 3. In WinSCP, refresh the view. Verify there are files with the format below:
    ...
    ...     - 8. Erase study by sending erasestudy command.
    ...     - 9. Verify device silently reboots after erasing study data.
    [Tags]  BTDV-PATIENTEVENT-12    manual
    Manual Should Be Passed     ${TEST_NAME}    ${TEST_DOCUMENTATION}
