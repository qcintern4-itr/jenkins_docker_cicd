...
try:
    ...
    from project_cardiac_portal.config.web_locators import devices_bioheart_report as locators
...
class DevicesBioheartReport(WebPage):
    """
    Handle Devices > Bioheart Report
    """
    ...
    def navigate_to_devices_bioheart_report(self):
        self.browser.open_page(self.URL + '/heartmonitor/bioheart')

    def filter_devices_bioheart_report(self, by_patient=None, by_status=None, search_by=None, text_search=None):
        if by_patient:
    ...
    def get_devices_bioheart_report(self, row=None):
    ...
    def _on_item(self, **on):
        if 'row' in on:
            return int(on['row']) - 1
        all_data = self.get_devices_bioheart_report()
